...
function priceDowngradeRangeSlider(){
    $("#pay_for_downgrade_users").attr("disabled","disabled");
    ...
    var plan_total_downgrade_users = pln.plan_total_downgrade_users;
    ...
    //alert(pln.plan_title);
    $("#downgrade-plan-holder").text('Plan:');
    $("#downgrade-device-holder").text(pln.plan_title+' Users');
    $(".info-price").html(pln.plan_currency_symbol+' '+ plan_price +' per user/ month');
    
    new Dragdealer('pr-downgrade-slider', {
        handleClass: "handle",
        animationCallback: function(x, y) {
            ...
            if(slider_value > plan_total_no_users){
                ...
                if( plan_total_downgrade_users > 0 && slider_value == 0){
                    slider_value = plan_total_downgrade_users;
                    $(".handle").css("left", "" + slider_value + "px");
                }
            }
            if( slider_value == 0 || slider_value == plan_total_no_users){
                $("#pay_for_downgrade_users").attr("disabled","disabled");
                if( slider_value == plan_total_no_users ){
                    show_error_message('response_messages_downgrade_users', "Sorry, you can not downgrade more than or euqal to Total Paid Users");
                }
            } else {
                $("#pay_for_downgrade_users").attr("disabled",false);
                
            }
            //$("#pr-downgrade-slider .value").text(slider_value);
            $(".downgrade_users_in_plan").html(slider_value);
            $(".totalusers-number").text(slider_value);            
            var stripe_width = slider_value + 1;
            $(".stripe-downgrade").css("width", "" + (stripe_width/10) + "%");
            if(stripe_width <= 10){ $(".stripe-downgrade").css("width", "" + (stripe_width/10) + "%"); }
            if(stripe_width >= 20){ $(".stripe-downgrade").css("width", "" + (stripe_width/10) + "%"); }
            if(stripe_width >= 30){ $(".stripe-downgrade").css("width", "" + (stripe_width/7) + "%"); }
            if(stripe_width >= 50){ $(".stripe-downgrade").css("width", "" + (stripe_width/7) + "%"); }
            if(stripe_width >= 100){ $(".stripe-downgrade").css("width", "" + (stripe_width/8) + "%"); }
            if(stripe_width >= 200){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }

            if(stripe_width >= 300){ $(".stripe-downgrade").css("width", ""+(stripe_width/9)+"%"); }

            if(stripe_width >= 350){ $(".stripe-downgrade").css("width", ""+(stripe_width/9)+"%"); }

            if(stripe_width >= 400){ $(".stripe-downgrade").css("width", ""+(stripe_width/9)+"%"); }
            
            if(stripe_width >= 450){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }

            if(stripe_width >= 500){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }
            
            if(stripe_width >= 600){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }
            
            if(stripe_width >= 700){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }

            if(stripe_width >= 800){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }

            if(stripe_width >= 900){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }

            if(stripe_width >= 950){ $(".stripe-downgrade").css("width", ""+(stripe_width/10)+"%"); }
