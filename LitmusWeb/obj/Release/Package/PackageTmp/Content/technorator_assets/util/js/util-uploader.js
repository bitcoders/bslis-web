(function($){
	...
	$.fn.extend({
		...
		uploader : function(options , attachment)
		{
		    ...
		    $fileupload
				.bind('fileuploadadd', function (e, data) { $this.trigger('upload-start' , [data]); })
				.bind('fileuploaddone', function (e, data) { $this.trigger('upload-end' , [data]); })
				.bind('fileuploadprogress', function (e, data) { $this.trigger('upload-progress' , [data]); });
		    ...
		    function show_attachment($this , attachment)
		    {
	        	...
	        	$('img.thumb' , $this).attr('src' , attachment['render_url'] + '/' + (options['size'].length ? options['size'] : ''));
	        	...
	        	if($parent.length > 0)
	        	{
	        		...
	        		{
	        			var a = '<a href="' + attachment['download_url'] + '" class="download">' + (attachment['name'] ? attachment['name'] : "Download") + '</a>';
	        			$parent.append(a);
	        		}
	        	}
	        	...
	        	$this.trigger('attachment_shown' , [attachment]);
